...
# Load data 
data <- read.csv(
  "E:\\Documents\\30day\\25\\Bayer_Leverkusen_Squad.csv",
  sep = ";",
  header = TRUE,
...
# PCA helps reduce dimensionality and understand variable contributions to each component
pca_result <- prcomp(data_scaled, scale. = FALSE)  # Data already scaled with scale() earlier

# Extract loadings (coefficients showing how variables contribute to each component)
loadings <- pca_result$rotation

# Print summary of PCA to see variance explained by each component (used for axis labels)
summary(pca_result)

# Loadings were used to interpret PC1 as "Participation" (dominated by Minutes and StartXI) and PC2 as "Age & Discipline" (dominated by Age and Yellow_Card)
...
final_plot <- p + 
  ...
  labs(
    ...
    y = "Dimension 2: Age & Discipline (20.3% variance)" 
  ) +
  theme(
